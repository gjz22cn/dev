...
export const Calendar: FC<CalendarProps> = ({ data, onTileClick, onItemClick, topBarWidth }) => {
  ...
  const [searchPersonPhrase, setSearchPersonPhrase] = useState("");
  ...
  const debouncedFilterPersonData = useRef(
    debounce((dataToFilter: SchedulerData, enteredSearchPersonPhrase: string) => {
      reset();
      setFilteredData(
        dataToFilter
          .map((item) => ({
            ...
            data: item.data.filter((dataItem) =>
              dataItem.subtitle.toLowerCase().includes(enteredSearchPersonPhrase.toLowerCase())
            )
          }))
  ...
  const handlePersonSearch = (event: ChangeEvent<HTMLInputElement>) => {
    const phrase = event.target.value;
    setSearchPersonPhrase(phrase);
    debouncedFilterPersonData.current.cancel();
    debouncedFilterPersonData.current(data, phrase);
  };
  ...
  useEffect(() => {
    if (searchPhrase || searchPersonPhrase) return;

    setFilteredData(data);
  }, [data, searchPhrase, searchPersonPhrase]);
  ...
  return (
    <StyledOuterWrapper>
      <LeftColumn
        ...
        searchPersonInputValue={searchPersonPhrase}
        onSearchInputChange={handleSearch}
        onPersonSearchInputChange={handlePersonSearch}
